...
//this function reads the value at the channel variable passed into it (we're using pin0 of the ADC, note not pin0 of the microcontroller)
...
//	unsigned char i;
	...
	while(1)
	{
		...
		//{
			...
			y=y*100;//multiply voltage in volts by 100 to convert to kelvin
			y=y-273;//subtract 273 to convert to celsius
			
			
			//do i still need to print it, or just "transmit" it through the serial port
			
			
			//printing i and y at the %d and %5.3f embedded format tags
			// %5.3 means print 5 characters minimum (value not truncated if larger than that) and 3 digits after decimal point
			//printf("V%d=%5.3f ", 0, y); 
